...
class TableModel extends Model{
	...
	public function addTable(){
		...
		$table_name = C("DB_PREFIX").$this->data["name"];
		if($table_type == 0){
			//列表
			$sql = <<<sql
...
sql;
	...
	public function delTable($rid){
		$table_list = M()->query("select * from ".C("DB_PREFIX")."table where id = '".$this->options["where"]["id"]."'");
		$table_name = C("DB_PREFIX").$table_list[0]["name"];
		$sql = <<<sql
DROP TABLE {$table_name};
sql;
		M()->query($sql);
		$id = $this->options["where"]["id"];
		M()->execute("delete * from ".C("DB_PREFIX")."field where tbl_id = {$id}");
		return $this->delete();
	}
