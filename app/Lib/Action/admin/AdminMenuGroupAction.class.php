...
class AdminMenuGroupAction extends AdminAction {
	...
	public function index(){
		...
		$AdminMenuGroup = D("AdminMenuGroup");  // 实例化AdminMenuGroup对象
		import("ORG.Util.Page");                    // 导入分页类
		$count      = $AdminMenuGroup->where($map)->count();   // 查询满足要求的总记录数
		...
		$list = $AdminMenuGroup->where($map)->limit($Page->firstRow.','.$Page->listRows)->order("id desc")->select();
	...
	public function add(){
		$AdminMenuGroup = D("AdminMenuGroup"); // 实例化AdminMenuGroup对象
		if(IS_POST){
			if (!$AdminMenuGroup->create()){
				// 如果创建失败 表示验证没有通过 输出错误提示信息
				$this->error($AdminMenuGroup->getError());
			}else{
				// 验证通过 可以进行其他数据操作
				$re = $AdminMenuGroup->addAdminMenuGroup();
				if ($re !== false) {
					$rurl = base64_decode($_GET["rurl"]);
					$rurl = empty($rurl)?"/admin/admin_menu_group":$rurl;
	...
	public function edit(){
		...
		if(IS_POST){
			if (!$AdminMenuGroup->create()){
				...
				// 验证通过 可以进行其他数据操作
				$re = $AdminMenuGroup->saveAdminMenuGroup();
				if ($re !== false) {
					$rurl = base64_decode($_GET["rurl"]);
					$rurl = empty($rurl)?"/admin/admin_menu_group":$rurl;
					$this->success('数据保存成功！', $rurl);
			...
			$map["id"] = $id;
			$doc = $AdminMenuGroup->where($map)->find();
			$this->assign('doc',$doc);
			$this->display();
	...
	public function del(){
		...
		if(IS_POST){
			...
			$map = array();
			$map["id"] = $id;
			$re = $AdminMenuGroup->where($map)->deleteAdminMenuGroup();
			if ($re !== false) {
				$rurl = base64_decode($_GET["rurl"]);
				$rurl = empty($rurl)?"/admin/admin_menu_group":$rurl;
